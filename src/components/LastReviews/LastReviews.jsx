...
import { toggleLike } from "../../redux/reviews/reviewsSlice";
...
export const LastReviews = () => {
  ...
  const { reviews, likedReviews } = useSelector((state) => state.reviews);
  ...
  const calculateMaxSliderValue = useCallback(() => {
    if (listRef.current && reviews.length > 0) {
      const listElement = listRef.current;
      const containerElement = listElement.parentElement;
      
      if (!containerElement) return;
      
      const containerWidth = containerElement.offsetWidth || containerElement.clientWidth || 0;
      const listWidth = listElement.scrollWidth || listElement.offsetWidth || 0;
      
      const maxScroll = Math.max(0, listWidth - containerWidth);
      
      setMaxSliderValue(maxScroll);
      
      setSliderValue((prevValue) => {
        if (prevValue > maxScroll) {
          return 0;
        }
        return prevValue;
      });
  ...
  const handleSubmitReview = (reviewData) => {
    ...
    dispatch(addReviewThunk(reviewWithDate));
    setIsModalOpen(false);
  };
  ...
  const handleLike = (reviewId) => {
    ...
    const isLiked = Array.from(likedReviews).includes(reviewId);
    const newLikesCount = isLiked ? (review.likes || 0) - 1 : (review.likes || 0) + 1;

    dispatch(toggleLike(reviewId));
    dispatch(updateReviewLikesThunk({ id: reviewId, likes: newLikesCount }));
  };
        ...
        <div className={style.reviews__containerReviews}>
          <div className={style.reviews__content}>
            ...
            <div className={style.reviews__logoSofa}>
              <svg
              className={style.reviews__logoSofaSvg}
            ...
            <div className={style.reviews__info}>
              ...
              <p className={style.reviews__text}>Базовано на відгуках: {reviews.length}</p>
            </div>
            ...
            >
              {reviews.map((review) => (
                <li key={review.id} className={style.reviews__item}>
                  <div>
                    <div className={style.reviews__iconBox}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="22"
                        height="22"
                    ...
                    <div className={style.reviews__actions}>
                      <button
                        className={`${style.reviews__likeButton} ${
                          likedReviews.includes(review.id) ? style.liked : ""
                        }`}
